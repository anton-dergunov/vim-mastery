...
| 8. Registers and putting | Unnamed `""`; yank `"0`; numbered `"1`–`"9`; named `"a`–`"z`; append with `"A`–`"Z`; black-hole `"_`; small delete `"-`; clipboard `"+` and `"*`; expression `"=`; read-only registers; `p P gp gP`; `:registers` as inspection | Units 1–6, especially `y d c p` | Preserve yanks, select storage deliberately, reuse multiple snippets, and understand why delete/change affects later puts | Delete without overwriting a yank; paste the previous yank after another edit; collect lines into a named register; put after the cursor and leave it positioned usefully | Core through named/black-hole registers; clipboard, expression, and read-only registers are advanced or host-dependent |
