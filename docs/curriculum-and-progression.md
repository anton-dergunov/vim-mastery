...
| 5. Precision motions and search | `f F t T ; ,`; `/ ? n N`; `* # g* g#`; `d/pat` `y/pat` `c?pat`; `gn gN`; `%`; `(`, `)`, `{`, `}` | Units 1–4 | Select the smallest reliable motion for nearby punctuation, repeated text, matching delimiters, sentences, and paragraphs | Delete until a quote; repeat a comma find; change the next search match; delete up to the next match; jump between brackets; move by paragraphs in prose or comments | Core. Search and pair matching remain text-based rather than IDE-semantic, and sentence motions are marked optional |
