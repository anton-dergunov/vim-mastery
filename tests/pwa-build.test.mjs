...
test("production PWA precaches core media and streams optional animation and scene variants", () => {
  ...
  // And the field notes. These are the only place the product describes
  // :argdo, quickfix and scripted Vim at all, so an offline build that drops
  // them loses the whole command-line story with no other symptom.
  assert.equal(
    readFileSync(join(dist, "content", "field-notes.json"), "utf8"),
    readFileSync(join(rootPath, "content", "field-notes.json"), "utf8"),
  );
  assert.match(worker, /content\/field-notes\.json/);
  // The mastery digest is generated by the build rather than copied, so this
  // checks the emitted file against the checked-in one that the content suite
  // holds to its source.
  assert.equal(
    readFileSync(join(dist, "content", "mastery-index.json"), "utf8"),
  ...
  assert.equal(media.core.length, 90 - pendingStoryArt);
